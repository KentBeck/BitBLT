 ...
 */
async function runSharedBufferBenchmark(
  width,
  height,
  pattern,
  iterations = null
) {
  // Determine iterations based on image size if not provided
  if (iterations === null) {
    const pixels = width * height;
    if (pixels > 4000000) iterations = 5; // For very large images (>4M pixels)
    else if (pixels > 1000000) iterations = 10; // For large images (>1M pixels)
    else if (pixels > 250000)
      iterations = 15; // For medium-large images (>250K pixels)
    else iterations = 20; // For smaller images
  }
  ...
  console.log(
    `Average pixels per microsecond: ${avgPixelsPerMicro.toFixed(
      2
...
async function runAllSharedBufferBenchmarks() {
  ...
  // Print header
  console.log(
    "| Size | Total Pixels | Avg Time (μs) | Avg Pixels/μs | Max Pixels/μs |"
  );
  console.log(
    "|------|--------------|---------------|---------------|---------------|"
  );
  ...
  for (const result of results) {
    console.log(
      `| ${result.width}x${
        result.height
      } | ${result.totalPixels.toLocaleString()} | ${result.avgTime
        .toFixed(2)
        .padStart(13)} | ${result.avgPixelsPerMicro
        .toFixed(2)
        .padStart(13)} | ${result.maxPixelsPerMicro.toFixed(2).padStart(13)} |`
    );
  }
}
