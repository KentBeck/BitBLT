...
function createFunctionSection() {
  ...
  // Combine all parts
  const functionContent = concatUint8Arrays([
    count,
    funcType
  ]);
 ...
 * @returns {Uint8Array} - The memory section
 */
function createMemorySection() {
  // Count of memories
  const count = encodeULEB128(1);
  
  // Memory limits (min=1, max=unspecified)
  const limits = new Uint8Array([0x00, 0x01]); // 0x00 = no max, 0x01 = min pages
  
  // Combine all parts
  const memoryContent = concatUint8Arrays([
    count,
    limits
  ]);
  
  return createSection(SectionId.MEMORY, memoryContent);
}
...
function createExportSection() {
  // Count of exports
  const count = encodeULEB128(2); // Export function and memory
  ...
  // Export "memory"
  const memNameLen = encodeULEB128(6); // "memory" length
  const memName = new Uint8Array([0x6d, 0x65, 0x6d, 0x6f, 0x72, 0x79]); // "memory"
  const memExportType = new Uint8Array([0x02]); // memory export
  const memIndex = encodeULEB128(0); // memory index 0
  
  // Combine all parts
  const exportContent = concatUint8Arrays([
    count,
    funcNameLen, funcName, funcExportType, funcIndex,
    memNameLen, memName, memExportType, memIndex
  ]);
...
function createCodeSection(options) {
  ...
  // Combine all parts
  const codeContent = concatUint8Arrays([
    count,
    func
  ]);
...
function generateBitBLTFunctionBody(options) {
  ...
  const localTypes = concatUint8Arrays([
    encodeULEB128(1), new Uint8Array([ValueType.I32]), // srcWidthInUint32
    encodeULEB128(1), new Uint8Array([ValueType.I32]), // dstWidthInUint32
    encodeULEB128(1), new Uint8Array([ValueType.I32]), // y
    encodeULEB128(1), new Uint8Array([ValueType.I32]), // srcYPos
    encodeULEB128(1), new Uint8Array([ValueType.I32]), // dstYPos
    encodeULEB128(1), new Uint8Array([ValueType.I32]), // x
    encodeULEB128(1), new Uint8Array([ValueType.I32])  // srcBit
  ]);
