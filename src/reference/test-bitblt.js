// Import the bitblt function for packed pixels (32 pixels per Uint32)
...
function runTest() {
  ...
  for (let y = 0; y < 8; y++) {
    for (let x = 0; x < 8; x++) {
      ...
      const actualValue = getPixel(dstBuffer, 8, x, y);
      if (actualValue !== expectedValue) {
        passed = false;
        console.log(
          `Failed at (${x}, ${y}): expected ${expectedValue}, got ${actualValue}`
        );
      }
    }
  }
